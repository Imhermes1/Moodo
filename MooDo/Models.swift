...
struct Task: Identifiable, Codable, Equatable {
    ...
    var priority: TaskPriority // Base priority set by user/AI
    ...
    var notes: [TaskNote]
    var sourceThoughtId: UUID? // Reference to originating thought
    
    // MARK: - Dynamic Priority System
    var dynamicPriority: TaskPriority {
        // If task is completed, priority doesn't matter
        if isCompleted { return priority }
        
        // If no deadline, keep original priority
        guard let deadline = reminderAt ?? deadlineAt else { 
            return priority 
        }
        ...
        switch daysUntil {
        case ...0:     return .high    // Overdue or today
        case 1:        return .high    // Tomorrow  
        case 2...3:    
            // Escalate low to medium, medium/high stay high
            return priority == .low ? .medium : .high
        case 4...7:    
            // At least medium priority for this week
            return priority == .low ? .medium : priority
        default:       
            return priority  // Keep original for future tasks
        }
    }
    
    var isEscalated: Bool {
        return dynamicPriority.rawValue != priority.rawValue
    }
    
    var priorityDescription: String {
        if isEscalated {
            switch dynamicPriority {
            case .high: return "High (Due Soon)"
            case .medium: return "Medium (This Week)"  
            case .low: return priority.displayName
            }
        }
        return dynamicPriority.displayName
    }
...
class TaskManager: ObservableObject {
    ...
    @Published var completionCardTask: Task? = nil
    let taskScheduler = TaskScheduler()
    let eventKitManager = EventKitManager()
    
    // Bidirectional sync with thoughts
    weak var thoughtsManager: ThoughtsManager? // Weak reference to avoid circular reference
    var isUpdatingFromThought = false // Flag to prevent infinite sync loops
    ...
    func updateTask(_ task: Task) {
        if let index = tasks.firstIndex(where: { $0.id == task.id }) {
            ...
            // Sync with linked thought if description changed (avoid infinite loop)
            if !isUpdatingFromThought && task.description != oldTask.description,
               let sourceThoughtId = task.sourceThoughtId,
               let thoughtsManager = self.thoughtsManager,
               let thoughtIndex = thoughtsManager.thoughts.firstIndex(where: { $0.id == sourceThoughtId }) {
                
                var updatedThought = thoughtsManager.thoughts[thoughtIndex]
                // Update thought content by prepending title if needed
                let taskDesc = task.description ?? ""
                let newContent = updatedThought.title.isEmpty ? taskDesc : "\(updatedThought.title)\n\n\(taskDesc)"
                updatedThought.content = newContent
                
                // Update the thought directly without triggering sync back
                thoughtsManager.thoughts[thoughtIndex] = updatedThought
                thoughtsManager.saveThoughts()
                thoughtsManager.saveToCloud()
            }
    ...
    func toggleTaskCompletion(_ task: Task) {
        ...
        if updatedTask.isCompleted {
            ...
            // Present completion card instead of sheet
            completionCardTask = updatedTask
        } else {
    ...
    func finalizeTaskCompletion(_ task: Task, mood: MoodType?) {
        ...
        // Record learning data for mood-emotion combination
        recordTaskCompletionForLearning(updatedTask)
        ...
        moodPickerTask = nil
        completionCardTask = nil
    }
    
    private func recordTaskCompletionForLearning(_ task: Task) {
        // Get the mood when the task was completed
        let completionMood = task.completedMood ?? getCurrentMood()
        
        // Create learning data structure
        let learningData = TaskCompletionData(
            taskId: task.id,
            ...
            actualCompletionTime: calculateActualCompletionTime(for: task),
            wasSuccessful: true, // Task was completed successfully
            completedAt: Date(),
        ...
        // Update emotion-mood success rates
        updateEmotionMoodSuccessRate(emotion: task.emotion, mood: completionMood)
    }
    
    private func getCurrentMood() -> MoodType {
        // Get current mood from UserDefaults or default
        if let data = UserDefaults.standard.data(forKey: "SavedMoodEntries"),
           ...
           let latestMood = entries.sorted(by: { $0.timestamp > $1.timestamp }).first {
            return latestMood.mood
        }
        return .calm
    }
    
    private func calculateActualCompletionTime(for task: Task) -> Int? {
        // Calculate time from creation to completion
        guard let completedAt = task.completedAt else { return nil }
        let timeInterval = completedAt.timeIntervalSince(task.createdAt)
        return Int(timeInterval / 60) // Return minutes
    }
    
    private func saveLearningData(_ data: TaskCompletionData) {
        var existingData = loadExistingLearningData()
        existingData.append(data)
        
        // Keep only recent data (last 200 completions for performance)
        if existingData.count > 200 {
            existingData = Array(existingData.suffix(200))
        }
        
        // Save to UserDefaults
        if let encoded = try? JSONEncoder().encode(existingData) {
            UserDefaults.standard.set(encoded, forKey: "MoodoTaskLearningData")
        }
    }
    
    private func loadExistingLearningData() -> [TaskCompletionData] {
        guard let data = UserDefaults.standard.data(forKey: "MoodoTaskLearningData"),
              let decoded = try? JSONDecoder().decode([TaskCompletionData].self, from: data) else {
            return []
        }
        return decoded
    }
    
    private func updateEmotionMoodSuccessRate(emotion: TaskEmotion, mood: MoodType) {
        let key = "EmotionMoodSuccess_\(emotion.rawValue)_\(mood.rawValue)"
        let currentData = UserDefaults.standard.dictionary(forKey: key) as? [String: Any] ?? [:]
        
        let completions = (currentData["completions"] as? Int) ?? 0
        let successes = (currentData["successes"] as? Int) ?? 0
        
        let newData: [String: Any] = [
            "completions": completions + 1,
            "successes": successes + 1,
            "successRate": Double(successes + 1) / Double(completions + 1),
            "lastUpdated": Date()
        ]
...
class ThoughtsManager: ObservableObject {
    ...
    weak var taskManager: TaskManager? // Weak reference to avoid circular reference
    ...
    func updateThought(_ thought: Thought) {
        if let index = thoughts.firstIndex(where: { $0.id == thought.id }) {
            let oldThought = thoughts[index]
            thoughts[index] = thought
            saveThoughts()
            saveToCloud()
            
            // Sync with linked task if content changed
            if thought.content != oldThought.content,
               let linkedTaskId = thought.linkedTaskId,
               let taskManager = self.taskManager,
               let taskIndex = taskManager.tasks.firstIndex(where: { $0.id == linkedTaskId }) {
                
                var updatedTask = taskManager.tasks[taskIndex]
                // Update task description with thought content (minus title)
                let contentWithoutTitle = extractContentWithoutTitle(from: thought.content, title: thought.title)
                updatedTask.description = contentWithoutTitle
                
                // Prevent infinite sync loop by temporarily setting a flag
                taskManager.isUpdatingFromThought = true
                taskManager.updateTask(updatedTask)
                taskManager.isUpdatingFromThought = false
            }
        }
    }
...
struct TaskCompletionData: Codable {
    let taskId: UUID
    let emotion: TaskEmotion
    let priority: TaskPriority
    let dynamicPriority: TaskPriority
    let category: TaskCategory
    let mood: MoodType
    let estimatedTime: Int?
    let actualCompletionTime: Int?
